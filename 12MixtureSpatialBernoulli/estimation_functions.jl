...
function M_step_vfast!(α, dists, y::AbstractVecOrMat, wp::AbstractMatrix{<:Real}, γ, method::PairwiseClassicEM; order=nothing, QMC_m=100, maxiters=1000)
    α[:] = mean(γ, dims=1)
    ...
    if isnothing(order)
        @threads for k in eachindex(eachcol(γ))
            γₖ = eachcol(γ)[k]
            dists[k] = fit_mle_vfast(dists[k], y, wp, γₖ, maxiters=maxiters)
        end
    else
        messages = Vector{String}(undef, length(eachcol(γ)))
        ...
        @threads for k in eachindex(eachcol(γ))
            messages[k] = "Thread ID: $(Threads.threadid()) handling index: $k"
            γₖ = eachcol(γ)[k]
            dists[k] = fit_mle_vfast(dists[k], y, wp, γₖ; order=dists[k].order, maxiters=maxiters)
        end
    end
    println.(messages)
...
function fit_mle_vfast!(
    ...
    for it = 1:maxiter
        ...
        @show [dists[k].range for k in 1:length(dists)]
        # M-step
        M_step_vfast!(α, dists, y, wp, γ, method; order=order, maxiters=maxiter_m)
