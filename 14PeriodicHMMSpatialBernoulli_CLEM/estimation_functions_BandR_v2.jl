...
#                 ξ_sum = 0.0
#                 for j in 1:K
#                     ξ[n,i, j,ii,jj] = αi * At[i, j] * exp(LL[n+1, j, ii, jj] - m) * β[n+1, j, ii, jj]
#                     ξ_sum += ξ[n,i, j,ii,jj]
#                 end
...
# -----------------------------
# Build JuMP model for A
# -----------------------------
function model_for_A(s_ξ::AbstractArray, d::Int; silence=true)
    println("modelèfor_A")
    T, K= size(s_ξ)
    @assert K > 1 "To define a transition matrix, need K ≥ 2, here K = $K"
    ...
    # P_kl polynomial
    @NLexpression(model, Pkl[t=1:T, l=1:K-1],
        sum(trig[t][j] * pklj_jump[l,j] for j=1:length(trig[t]))
    )

    # smoothing parameters (counts from E-step)
    @NLparameter(model, s_πkl[t=1:T, l=1:K-1] == s_ξ[t,l])
    @NLparameter(model, s_πk[t=1:T] == sum(s_ξ[t,l] for l=1:K))
    ...
    @NLobjective(model, Max,
        sum(
            sum(s_πkl[t,l]*Pkl[t,l] for l=1:K-1) - 
            s_πk[t]*log1p(sum(exp(Pkl[t,l]) for l=1:K-1))
            for t=1:T
        )
    )
    ...
    model[:s_πkl] = s_πkl
    model[:s_πk] = s_πk
    model[:pklj_jump] = pklj_jump

    return model
end

# -----------------------------
# Fit one row of A (per k) using MLE
# -----------------------------
function fit_mle_one_A(θᴬ, model::Model, s_ξ_k::AbstractArray{<:AbstractFloat,2}; warm_start=true)
    T, K = size(s_ξ_k)
    pklj_jump = model[:pklj_jump]
    s_πkl = model[:s_πkl]
    s_πk  = model[:s_πk]

    # update JuMP NLparameters
    for t in 1:T
        set_value(s_πk[t], sum(s_ξ_k[t,l] for l=1:K))
        for l in 1:(K-1)
            set_value(s_πkl[t,l], s_ξ_k[t,l])
        end
    end

    # warm start if desired
    if warm_start
        set_start_value.(pklj_jump, θᴬ[:, :])
    end
...
function update_A_CLEMbis!(
    A::AbstractArray{<:AbstractFloat,3},         # K × K × T
    θᴬ::AbstractArray{<:AbstractFloat,3},       # K × (2d+1) × (K-1)
    ξ::AbstractArray{<:AbstractFloat,5},        # N × K × K × D × D
    s_ξ::AbstractArray{<:AbstractFloat,3},      # T × K × K
    α::AbstractArray{<:AbstractFloat,4},        # N × K × D × D
    β::AbstractArray{<:AbstractFloat,4},        # N × K × D × D
    LL::AbstractArray{<:AbstractFloat,4},       # N × K × D × D
    n2t::AbstractVector{Int},                   # map from n to t
    n_in_t::Vector{Vector{Int}},                # indices n belonging to each t
    model_A::Model,
    wp::AbstractMatrix,                          # D × D weights
    pairwise_indices2=Tuple.(findall(wp .> 0));
    warm_start=true
)
    N, K, _, D = size(LL)
    ...
    println("Computing per-pair ξ probabilities...")
    ...
    Threads.@threads for n in 1:(N-1)
        ...
        for (ii,jj) in pairwise_indices2
            ...
            for i in 1:K, j in 1:K
                ξ_pair[i,j] = α[n,i,ii,jj] * At[i,j] * exp(LL[n+1,j,ii,jj]-m) * β[n+1,j,ii,jj]
            end
            ...
            invZ = w / Z
            for i in 1:K, j in 1:K
                ξ[n,i,j,ii,jj] = ξ_pair[i,j] * invZ
            end
        end
    end

    println("Aggregating ξ into s_ξ for M-step...")

    # ---- aggregate ξ over time into s_ξ ----
    for t in 1:T
        for (k,l) in Iterators.product(1:K, 1:K)
            s_ξ[t,k,l] = sum(ξ[n,k,l,ii,jj] for n in n_in_t[t], (ii,jj) in pairwise_indices2)
        end
    end

    # ---- normalize rows per state i ----
    for t in 1:T
        for i in 1:K
            row_sum = sum(s_ξ[t,i,:])
            if row_sum > 0
