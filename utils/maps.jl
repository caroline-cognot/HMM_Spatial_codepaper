#  ©2026 EDF - <caroline.cognot@agroparistech.fr>
...
# add adjacency plot between locations.
    ...
    fig = GeoMakie.with_theme(GeoMakie.theme_latexfonts(), fontsize=fontsize) do
        fig = GeoMakie.Figure()
        ax = GeoMakie.GeoAxis(fig[1, 1], dest="+proj=merc", title=title, xgridvisible=false, ygridvisible=false, xticklabelsvisible=true, yticklabelsvisible=true, xticksvisible=false, yticksvisible=false)
        GeoMakie.xlims!(ax, LON_min, LON_max)
        GeoMakie.ylims!(ax, LAT_min, LAT_max)
        GeoMakie.poly!(ax, ocean_ne.geometry)
        GeoMakie.lines!(ax, GeoMakie.to_multilinestring.(borders_ne.geometry); color=:black, linestyle=:dash, linewidth=0.75)
        ...
        if w !== nothing
            ...
            for i in 1:length(LON_idx)
                for j in 1:length(LON_idx)
                    if w[i, j] != 0
                        line_points = [GeometryBasics.Point2(LON_idx[i], LAT_idx[i]), GeometryBasics.Point2(LON_idx[j], LAT_idx[j])]
                        GeoMakie.lines!(ax, line_points; color=:blue, linewidth=2 * scale * w[i, j])
                    end
                end
            end
            ...
            npair = sum(w .> 0)
            ntot = size(w)[1]^2
            GeoMakie.text!(ax, [(LON_min + 3)], [LAT_min + 1]; text="pairs = $npair/$ntot", color=:black, font=:bold, align=(:center, :top), fontsize=fontsize)
