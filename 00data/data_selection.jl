...
using Dates
begin
    station_all = CSV.read("./00data/ECA_blended_custom/stations.txt", DataFrame, header=18, normalizenames=true, ignoreemptyrows=true)
...
CSV.write("./00data/transformedECAD_locs.csv", DataFrame(my_locations, :auto), header=false)
CSV.write("./00data/transformedECAD_Yobs.csv", DataFrame(Yobs, :auto), header=false)
CSV.write("./00data/transformedECAD_locsdistances.csv", DataFrame(my_distance, :auto), header=false)
